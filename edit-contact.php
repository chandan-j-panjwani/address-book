...
if(isset($_POST['id']))
{
    ...
    else
    {
        ...
        if($image_has_been_changed)
        {
            ...
            {
                unlink($path_to_image . $old_file_name);
            }
        }
        else
        {
            //I have to copy the old extinsion in new image file name
            $temp = explode(".", $old_file_name);
            $old_file_extension = strtolower(end($temp));
            $image_name .= "." .$old_file_extension;
            if($old_file_extension === $image_name){}
            else
            {
                $old_file_path = $path_to_image . $old_file_name;
                $new_file_path = $path_to_image . $image_name;
                rename($old_file_path, $new_file_path);
            }
        }
    }

    // $query = "UPDATE contacts SET `first_name` = '$file_name', `last_name` = 'last_name', `email` = '$email', `birthdate` = '$birthdate', `telephone` = '$telephone', 
    //         `address` = '$address', `image_name` = '$image_name' WHERE id = $id";
...
?>
    <div class="container">
        <div class="row">
            <div class="col s12">
                <h1>Invalid Access to the Page!</h1>
            </div>
        </div>
    </div>
<?php
...
?>
        ...
        <div class="row">
            <div class="materialert error">
                ...
                There were some errors while inserting record! Please retry again!
...
?>
        ...
        <div class="row">
            ...
            <input type="hidden" value="<?= $id;?>" name = "id">
                ...
                <div class="row mb10">
                    <div class="col s2">
                        <img src="images/users/<?=$row[0]['image_name'];?>" id = "temp_pic" alt="" width="78px" height="78px">
                    </div>   
